<template>
  <main>
    ...
    <header class="relative py-24 bg-cover bg-center"
      style="background-image: url('https://raw.githubusercontent.com/ProGestionSoft/Files/main/SuitOps_Landing/Hero/terms.png')">
      ...
      <div class="relative max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 text-center text-BtW">
        <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 sm:mb-6" v-motion
          ...
          {{ $t('legalsPage.termsHeroTitle') }}
        </h1>
        <p class="text-base sm:text-lg md:text-xl max-w-2xl mx-auto">
          {{ $t('legalsPage.termsHeroSubtitle') }}
        </p>
      </div>
    </header>
...
// SEO
useHead({
  title: t('legalsPage.termsHeroTitle')
})
...
const fetchLegal = async () => {
  ...
  while (attempts < maxAttempts) {
    try {
      const response = await SuitOpsServices.getLegal('condition')
      ...
      if (attempts < maxAttempts) {
        ...
        error.value = t('common.loadError', { object: t('footer.termsOfUseLink') });
        loading.value = false
      }
    }
  }
