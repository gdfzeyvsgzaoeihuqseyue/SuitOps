  ...
  <main>
    ...
    <section class="container mx-auto p-8 sm:p-16 max-w-7xl">
      ...
      <div v-else>
        ...
        <div class="relative flex justify-center items-center md:hidden mb-12 sm:mb-16">
          <button @click="prevPlan" :disabled="currentPlanIndex === 0"
            class="absolute left-0 p-2 rounded-full bg-gray-200/50 hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed z-10">
            <IconChevronLeft class="h-6 w-6" />
          </button>
          
          <div v-if="selectedPlan"
            class="relative border-2 rounded-2xl p-6 sm:p-8 shadow-sm hover:shadow-lg transition-all w-full max-w-sm"
            :class="isPopular(selectedPlan) ? 'border-secondary bg-bgClr' : 'border-gray-200 bg-WtB'">
            <div v-if="isPopular(selectedPlan)"
              class="absolute -top-3 left-1/2 -translate-x-1/2 badge-popular text-xs sm:text-sm">
              {{ $t('pricingsPage.popularBadge') }}
            </div>
            <div class="mb-4 sm:mb-6 text-center">
              <h2 class="text-xl sm:text-2xl font-bold" :class="selectedPlan.name === 'GRATUIT'
                ? 'textClr'
                : isPopular(selectedPlan) ? 'text-secondary' : 'text-primary'">
                {{ selectedPlan.name }}
              </h2>
              <p class="text-sm sm:text-base">{{ selectedPlan.description }}</p>
            </div>
            <div class="mb-6 sm:mb-8">
              <div class="flex items-baseline gap-1 sm:gap-2 justify-center text-xl sm:text-2xl font-bold">
                <span>
                  {{ formatPrice(showAnnual ? selectedPlan.priceAnnual : selectedPlan.priceMonthly, selectedCurrency) }}
                </span>
              ...
              <div v-if="selectedCurrency !== 'XOF'" class="text-xs text-center mt-1">
                {{ formatPrice(showAnnual ? selectedPlan.priceAnnual : selectedPlan.priceMonthly, 'XOF') }}
                <p>
            ...
            <button class="w-full py-2 sm:py-3 rounded-lg font-medium transition-colors text-sm sm:text-base"
              :class="selectedPlan.name === 'GRATUIT'
              ...
              {{ selectedPlan.name === 'GRATUIT' ? $t('pricingsPage.freePlanButton') : $t('pricingsPage.choosePlanButton', {
                planName: selectedPlan.name
              }) }}
            </button>
            <ul class="mt-6 sm:mt-8 space-y-2 sm:space-y-3">
              <li v-for="feature in selectedPlan.features" :key="feature.feature.id" class="flex items-start">
          ...
          <button @click="nextPlan" :disabled="currentPlanIndex === groupedPlans.length - 1"
            class="absolute right-0 p-2 rounded-full bg-gray-200/50 hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed z-10">
            <IconChevronRight class="h-6 w-6" />
          </button>
        </div>

        <div class="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-12 sm:mb-16">
...
import {
  IconMoodCry, IconCheck, IconX, IconSquareRoundedCheck, IconSquareRoundedX,
...
// Gestion du carrousel pour mobile
const currentPlanIndex = ref(0);
const selectedPlan = computed(() => {
  if (groupedPlans.value.length > 0) {
    return groupedPlans.value[currentPlanIndex.value];
  }
  return null;
});

const nextPlan = () => {
  if (currentPlanIndex.value < groupedPlans.value.length - 1) {
...
// ... (toutes les fonctions existantes restent ici) ...
