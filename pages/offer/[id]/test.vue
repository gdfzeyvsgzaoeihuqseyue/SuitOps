<template>
  <main class="bg-gray-50 py-12 mt-8">
    <!-- Breadcrumb -->
    <useBreadcrumb :items="breadcrumbItems" />

    <!-- En-tête du formulaire -->
    <section class="max-w-4xl mx-auto px-6 text-center mb-8">
      <h1 class="text-3xl font-bold mb-2"> QUIZ à <span class="text-primary">{{ $route.params.id }}</span></h1>
    ...
    <section class="bg-white max-w-4xl mx-auto m-6 p-6 rounded-lg shadow-md border">
      <form @submit.prevent="submitApplication" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Téléphone 1 <span
                  class="text-danger">*</span></label>
              <input id="phone1" type="tel" required class="formStyle">
              <span v-if="errors.phoneNumber2" class="text-danger text-sm">{{ errors.phoneNumber1 }}</span>
            </div>
            <!-- Pays -->
            <div>
        ...
        <section class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1"> Structure <span
                    class="text-danger">*</span></label>
                <input v-model="exp.structure" type="text" required placeholder="Nom de la structure" class="formStyle">
              </div>
              <!-- Durée -->
              <div>
          ...
          <button @click="addExperience" type="button" class="flex items-center text-primary hover:text-secondary mb-4">
        ...
        <section class="bg-gray-50 p-4 rounded-lg">
          ...
          <div v-for="(lang, index) in formData.languages" :key="index" class="mb-4 p-4 border rounded">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <!-- Langue -->
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1"> Langue <span class="text-danger">*</span>
                </label>
        ...
        <section class="bg-gray-50 p-4 rounded-lg">
          ...
          <!-- CNI -->
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">CNI <span class="text-danger">*</span></label>
          ...
          <!-- CV -->
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">CV <span class="text-danger">*</span></label>
      ...
      <div v-if="showPreviewModal"
        ...
        <div class="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] flex flex-col">
          ...
          <div class="p-6 overflow-y-auto flex-1 space-y-6">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p class="font-medium">{{ formData.lastname }}</p>
                </div>
                <div>
                  <p class="text-sm">Prénom</p>
                  <p class="font-medium">{{ formData.firstname }}</p>
                </div>
                <div>
                  <p class="text-sm">Email</p>
                  <p class="font-medium">{{ formData.emailAddress }}</p>
                </div>
                <div>
                  <p class="text-sm">Téléphone</p>
                  <p class="font-medium">{{ formData.phoneNumber1 }}</p>
                </div>
              </div>
            </div>
            ...
            <!-- Expériences professionnelles -->
            <div class="bg-gray-50 p-4 rounded-lg">
              <h4 class="font-semibold mb-3 text-lg">Expériences professionnelles</h4>
              <div v-for="(exp, index) in formData.experiences" :key="index" class="mb-3">
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <p class="text-sm">Structure #{{ index + 1 }}</p>
                    <p class="font-medium">{{ exp.structure }}</p>
                  </div>
                  <div>
                    <p class="text-sm">Durée (mois)</p>
                    <p class="font-medium">{{ exp.duration }}</p>
                  </div>
                </div>
              </div>
            </div>
...
import { IconTrash, IconLoader, IconSend, IconEye, IconX } from '@tabler/icons-vue'
...
const validateBirthdate = () => {
  ...
  errors.value.birthdate = selectedDate >= today ? 'La date doit être antérieure à aujourd\'hui' : '';
};
...
const validatePhones = () => {
  if (!formData.phoneNumber1) {
    errors.value.phoneNumber1 = 'Ce champ est obligatoire';
    return false;
  }
