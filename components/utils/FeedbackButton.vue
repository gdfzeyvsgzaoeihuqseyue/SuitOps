<template>
  <div class="relative">
    <button @click="showModal = !showModal"
      class="fixed bottom-6 right-6 bg-primary text-WtB p-4 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110 z-50"
    ...
    <Transition name="menu-slide-in">
      <div v-if="showModal" ref="menuRef"
        class="fixed bottom-[100px] right-6 w-80 bg-ash rounded-xl shadow-2xl flex flex-col z-40 border border-ashAct">

        <div class="p-4 border-b border-hsa">
          <h2 class="text-lg font-bold">{{ t('feedback.title') }}</h2>
        </div>
        ...
        <div class="p-4 space-y-3 overflow-y-auto max-h-[400px]">
          ...
          <NuxtLink :to="localePath('/forum')" @click="showModal = false"
            class="w-full flex items-center justify-between p-3 border border-hsa rounded-lg hover:bg-ashAct transition-colors text-sm font-medium">
            <div class="flex items-center">
              <IconMessages class="h-5 w-5 text-primary mr-3" />
